...
Reload is a presentation-only 1.1-second lower/tilt/recover motion, adjustable on each external wrapper through the Third-person Reload fields. It reacts to the existing confirmed Reload action. After merging main (PR #62), gameplay reload uses its authoritative timed flow and authored owner animation; external motion does not perform the ammo transfer. Late joiners reconstruct equipped items and ongoing timed uses through existing state; completed one-shot actions are not replayed.
...
## Merge revision against main

Resolved the PR #62 overlap while retaining the newest first-person/world assets and the same rebuilt external meshes. Kept external grip placement, locomotion fixes, and use/recoil motion alongside main's authored owner handling, timed reload, audio, and combat. Earlier build and two-instance results above predate this combined revision; they are not a claim of a new combined host/client run.

Combined revision validation: Unity 6000.3.15f1 compilation completed; Console zero errors; 4/4 ExternalItemPresentationTests PlayMode tests and 18/18 RoadmapItemAssetTests EditMode tests passed. Inspected all nine equipable definitions and local/external/world mesh sources in Unity. No missing materials. Removed duplicate test assembly references introduced by automatic merge. A fresh two-instance host/client run and Windows build were not repeated for this merge revision.
