...
namespace TheSancturary.FusionPrototype.Tests
{
    public sealed class ItemCombatRuntimeTests
    {
        ...
        [UnityTest]
        public IEnumerator HitsHaveNoHurtVoiceAndDeathPlaysUploadedClipOnce()
        {
            foreach (bool henry in new[] { false, true })
            {
                var monster = SpawnMonster(henry);
                var settings = Definition("old_revolver").CombatSettings;
                Assert.That(Contact(monster, Definition("old_revolver")), Is.True);
                yield return new WaitForSeconds(.1f);
                Assert.That(Object.FindObjectsByType<AudioSource>(FindObjectsSortMode.None)
                    .Any(s => s.name == "Henry hurt scream" || s.name == "Geo hurt growl"), Is.False);
                Assert.That(settings.hitEffect.emission.GetBurst(0).count.constantMax, Is.EqualTo(9));
                Assert.That(Contact(monster, Definition("sawed_off_shotgun")), Is.True);
                yield return new WaitForSeconds(.1f);
                var voices = Object.FindObjectsByType<AudioSource>(FindObjectsSortMode.None)
                    .Where(s => s.name == "Monster death audio").ToArray();
                Assert.That(voices.Length, Is.EqualTo(1));
                Assert.That(voices[0].clip, Is.SameAs(settings.deathBurst));
                Assert.That(voices[0].clip.name, Is.EqualTo("monster_death_uploaded"));
                Assert.That(voices[0].volume, Is.EqualTo(1f));
                monster.Render();
                monster.Render();
                Assert.That(Object.FindObjectsByType<AudioSource>(FindObjectsSortMode.None)
                    .Count(s => s.name == "Monster death audio"), Is.EqualTo(1));
                yield return new WaitForSeconds(settings.deathBurst.length + .3f);
            }
        }
        [UnityTest]
